...
// OneClick functions
	TBK.prototype.initInscription = function(email, username, responseURL, callback) {
		var self = this;
		var data = {
			arg0: {
				"email": email,
		...
		this.soapClient.OneClickPaymentServiceImplService.OneClickPaymentServiceImplPort.initInscription(data, function(error, result, raw){
			if( self.config.DEBUG_LOGS ){
				console.log("TBK OUTPUT XML > "+raw);
			}

			if(!error && _this.verifySignature(raw)){
				callback(error, result, _this.soapClient.lastRequest);
			}else{
				callback({message : "Invalid signature or "+error}, result);
			}
		})
	...
	TBK.prototype.finishInscription = function(token, callback) {
		var self = this;
		var data = {
			arg0: {
				"token": token
		...
		this.soapClient.OneClickPaymentServiceImplService.OneClickPaymentServiceImplPort.finishInscription(data, function(error, result, raw){
			if( self.config.DEBUG_LOGS ){
				console.log("TBK OUTPUT XML > "+raw);
			}
	...
	TBK.prototype.authorize = function(username, tbkUser, amount, buyOrder, callback) {
		var self = this;
		var data = {
			arg0: {
				"username": username,
		...
		this.soapClient.OneClickPaymentServiceImplService.OneClickPaymentServiceImplPort.authorize(data, function(error, result, raw){
			if( self.config.DEBUG_LOGS ){
				console.log("TBK OUTPUT XML > "+raw);
			}

			if(!error && _this.verifySignature(raw)){
				callback(error, result, this.soapClient.lastRequest);
			}else{
				callback({message : "Invalid signature or "+error}, result);
			}
		})
	...
	TBK.prototype.removeUser = function(username, tbkUser, callback) {
		var self = this;
		var data = {
			arg0: {
				"username": username,
		...
		this.soapClient.OneClickPaymentServiceImplService.OneClickPaymentServiceImplPort.removeUser(data, function(error, result, raw){
			if( self.config.DEBUG_LOGS ){
				console.log("TBK OUTPUT XML > "+raw);
			}
	...
	TBK.prototype.codeReverseOneClick = function (buyorder, callback) {
		var self = this;
		var data = {
			arg0: {
				'buyorder': buyorder
		...
		this.soapClient.OneClickPaymentServiceImplService.OneClickPaymentServiceImplPort.codeReverseOneClick(
			data, function (error, result, raw) {
				if (self.config.DEBUG_LOGS) {
					console.log('TBK OUTPUT XML > ' + raw);
				}
